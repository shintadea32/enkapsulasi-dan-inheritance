/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package latihan;

/**
 *
 * @author I d e a P a d
 */
public class KipasAngin extends Toko {//menggunakan extends untuk menghubungkan kipas angin dengan toko
    int ukuran;//membuat tipe data int dengan variabel ukuran
    
    public void display(){//membuat method display
        System.out.println("Data barang yang di entry : ");//isi method display
    }
    public void setHarga(float harga){//membuat method setharga dengan parameter float harga
        super.harga = harga;//karena variabel berada di class lain, maka menggunakan super
        System.out.println("Harga : "+harga);//untuk menampilkan harga
    }
    public float getHarga(){//method non void
        return harga;//method non void menggunakan return
    }
    public void setTotal(int total){//membuat method settotal dengan parameter int total
        super.total = total;//karena variabel ada di class lain, maka menggunakan super
        System.out.println("Total : "+total);//untuk menampilkan total
    }
    public int getTotal(){//method non void
        return total;//method non void menggunakan return
    }
    public void setMerk(String merk){//membuat method setmerk dengan parameter string merk
        super.merk = merk;//karena variabel ada di class lain maka menggunakan super
        System.out.println("Merk : "+merk);//untuk menampilkan merk
    }
    public String getMerk(){//method non void
        return merk;//mathod non void menggunakan return
    }
    public void setUkuran(int ukuran){//membuat method setukuran dengan parameter int ukuran
        this.ukuran = ukuran;//karena variabel ada di class yang sama, maka menggunakan this
        System.out.println("Ukuran : "+ukuran);//untuk menampilkan ukuran
    }
    public int getUkuran(){//method non void
        return ukuran;//method non void menggunakan return
    }
}